...
//
export function wgslLoader(extensions = ["wgsl"]): Plugin {
 ...
 */
function load(filePath: string, visited = new Set<string>()): string {
    ...
    for (const line of lines) {
        // Look for lines like: #include "someFile.ext"
        // A simple pattern:  ^\s*#include\s+"([^"]+)"\s*$
