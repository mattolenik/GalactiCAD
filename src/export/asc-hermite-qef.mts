 ...
 */
export function applyAscHermiteQef(mesh: MeshData, grid: AscVoxelGrid, opts: AscHermiteQefOptions): AscHermiteQefResult {
    const verts = new Float32Array(mesh.verts)
    const tris = mesh.tris
    const vertCount = (verts.length / VERTEX_STRIDE_F32) | 0
    ...
    return { verts, tris, movedVertices }
}
