...
- **Build**: `make build`
- **Test**: `make test`
- **Validation / type check**: `make build` — use this to validate changes instead of `make check` or `npx tsc --noEmit`
...
2. For **`/_agent/render`** and **`/_logs`** after a headless render: ensure **`make start AGENT=true`** and **`.devserver.agent.run`**, then read **`port`** with **`jq -r .port .devserver.agent.run`**. To read the **interactive** editor ( **`make start`** ), use **`jq -r .port .devserver.run`** for **`/_sceneSource`** / **`/_agent/capture-testcase`**. For **`/_logs`**, prefer **`scripts/agentcli logs`** — it defaults to the **agent** devserver (**`--agent`**) and reads **`--browser`** from the interactive one (**`.devserver.run`**), each via its own run file, and defaults to **all levels (debug+)** so enabled modules aren't silently dropped (narrow with `--level warn` etc.). If you drop to raw shell **`curl`**, the default response uses the **`level=info`** threshold (drops `debug`); add `level=debug` or `only=…` when you need module/debug output. A **`503`** means **no browser tab is connected** (or the bridge timed out); an empty **`200`** means the connected tab's buffer had no matching entries — `agentcli logs` reports these distinctly. Do **not** guess a port, do **not** launch a browser yourself.
