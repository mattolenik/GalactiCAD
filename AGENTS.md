...
The CAD models are defined by JavaScript code that the user edits in-app using the CodeMirror code
...
- Polygon editing UX: double-clicking polygon2d, loft, union, or other cross-selectable symbols selects in the preview; the editor adds no occurrence/selection-match highlighting; polygon editing opens from a hover-only "Edit Polygon" menu shown over the shape's color icon (not the text, and not right-click); right-click uses the app's custom DOM context menu (Edit polygon / View Isolated / Insert shape); keep a safe-zone AABB between trigger and menu so it stays open while the cursor moves toward it.
