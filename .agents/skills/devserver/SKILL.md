...
- **Default for automation:** run **`make start AGENT=true`** when you need **`/_logs`**, **`/_sceneSource`**, **`/_agent/*`**, or a **self-contained** headless render. Read **`port`** from **`.devserver.agent.run`**.
- **Mirroring a human’s interactive tab** ( **`make start`** ): that browser talks to **`.devserver.run`**. To copy their scene + camera + export + mesh debug overlays into the agent stack, use **two ports** — capture from **`.devserver.run`**, render on **`.devserver.agent.run`** (see **Mirror interactive → agent** below).
- **Port:** read **`port`** from **`.devserver.agent.run`** only: `jq -r .port .devserver.agent.run`. If the file is missing after **`make start AGENT=true`**, the agent HTTP bridge is unavailable — **do not guess a port**.
...
- **Logs:** **`.devserver.agent.log`** (tail with **`make logs AGENT=true`**). **Stop:** **`make stop AGENT=true`**.

**Humans / interactive sessions** use **`make start`** and **`.devserver.run`**. Use that port for **`/_sceneSource`** and **`/_agent/capture-testcase`** when the goal is “what the user sees right now.” Use **`make start AGENT=true`** and **`.devserver.agent.run`** for headless **`/_agent/render`** and for **`/_logs`** after those renders (the interactive tab does not receive agent render RPCs).
...
- **Run file:** **`.devserver.run`** in the repo root (JSON: `pid`, `port`), written when **`make start`** runs (without **`AGENT=true`**).
...
- **Start:** **`make start AGENT=true`** ( **`AGENT=true`** ; default **PORT=7900** unless overridden; devserver may bind the next free port if busy — always read **`port`** from the run file).
- **Run file:** **`.devserver.agent.run`**. Read the port: `jq -r .port .devserver.agent.run`.
- **Logs file:** **`.devserver.agent.log`** ( **`make logs AGENT=true`** ).
- **Stop:** **`make stop AGENT=true`**.
...
1. Run **`make start AGENT=true`** if **`.devserver.agent.run`** is not present; read **`port`** from that file. **Do not** launch Chromium yourself—the agent devserver starts headless Chrome for the bridge.
...
Use this when the human runs **`make start`** and you want the **same** scene text, camera, viewport crop, **`meshExport`** levers, and **mesh debug overlay** flags as in their tab, then render and inspect logs **without** asking them to switch to the agent devserver.
...
3. **Agent stack:** run **`make start AGENT=true`** if **`.devserver.agent.run`** is absent; `agent_port=$(jq -r .port .devserver.agent.run)`.
...
1. Assign `port=$(jq -r .port .devserver.agent.run)` from the repo root (**agents**). If the file does not exist, run **`make start AGENT=true`** and retry once; if `jq` still errors or `port` is empty, **stop**—do not assume any default port. _(Interactive: use **`.devserver.run`** instead.)_
...
- **Agents:** **`jq -r .port .devserver.agent.run`** after **`make start AGENT=true`**. **Interactive:** **`.devserver.run`**. Then **`curl`** **`/_logs`**, **`/_sceneSource`**, and agent routes on that host/port.
