...
* INPUT GRAMMAR:
* NUMBER := m='-'? i=INT f=FRAC?
*     .value = number { return parseFloat((this.m??'') + this.i + (this.f??'')); }
* INT := '0|[1-9][0-9]*'
* FRAC := '\.[0-9]+'
* CHARS := '([^"\\\u0000-\u001F]|\\["\\/bfnrt])*'u
* // Whitespace
...
export class NUMBER {
    ...
    public value: number;
    constructor(m: Nullable<string>, i: INT, f: Nullable<FRAC>){
        this.m = m;
        this.i = i;
        this.f = f;
        this.value = ((): number => {
        return parseFloat((this.m??'') + this.i + (this.f??''));
        })();
    }
}
        ...
        return this.run<NUMBER>($$dpth,
            () => {
                ...
                if (true
                    ...
                    $$res = new NUMBER($scope$m, $scope$i, $scope$f);
                }
                return $$res;
            });
    }
