...
// lets the concrete `MeshExporter<T>` values share one record type. The worker
// re-normalizes the incoming tuning blob through each exporter anyway.
...
/** Look up the exporter for a kind, falling back to SFCC-RS for unknown values. */
export function getExporter(kind: ExporterKind | undefined): MeshExporter<unknown> {
    return EXPORTERS[kind ?? "sfcc-rs"] ?? EXPORTERS["sfcc-rs"]
}
