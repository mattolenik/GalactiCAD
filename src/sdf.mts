...
import { exportSDFToSTLGPU } from "./scene/export-stl.mjs"
...
export class SDFRenderer {
    ...
    #buildPreviewPipeline() {
        const shaderModule = this.#shaderCompiler.compile(previewShader, "Preview Window")
    ...
    async exportSTL(src: string): Promise<void> {
        this.build(src)
        const handle = await window.showSaveFilePicker({
            types: [{ description: "Binary STL", accept: { "application/vnd.ms-pki.stl": [".stl"] } }],
        })
        await exportSDFToSTLGPU(
            this.#device,
            {
                sliceCount: 200,
                width: 1024,
